...
# -*- coding: utf-8 -*-
"""
Перестановочный тест для CLR + Aitchison (без балансировки групп)
ВНИМАНИЕ: Этот скрипт показывает артефакт разного размера выборок!
Для корректного результата используйте perm_clr.py с балансировкой.
"""
...
# ============================================
print("=" * 60)
print("ПЕРЕСТАНОВОЧНЫЙ ТЕСТ (CLR + Aitchison) - БЕЗ БАЛАНСИРОВКИ")
print("=" * 60)
...
healthy_data = healthy_filt.T  # (3268, n_mag)
crc_data = crc_filt.T          # (662, n_mag)
...
print(f"   Здоровых после CLR: {healthy_clr.shape}")
print(f"   CRC после CLR: {crc_clr.shape}")

# Реальное расстояние (между ВСЕМИ здоровыми и ВСЕМИ CRC)
print("3. Вычисление реального расстояния...")
dgms_h_real = compute_diagram(healthy_clr)
dgms_c_real = compute_diagram(crc_clr)
...
# ============================================
n_permutations = 200
print(f"\n4. Перестановочный тест ({n_permutations} итераций)...")

# Объединяем все данные
all_data = np.vstack([healthy_clr, crc_clr])
n_healthy = healthy_clr.shape[0]
n_crc = crc_clr.shape[0]
labels = np.array([0]*n_healthy + [1]*n_crc)
...
for perm in range(n_permutations):
    ...
    # Формируем группы (без балансировки)
...
# ============================================
print("\n" + "=" * 60)
print("РЕЗУЛЬТАТЫ (БЕЗ БАЛАНСИРОВКИ)")
print("=" * 60)
...
with open("perm_results_clr_no_balance.txt", "w") as f:
    f.write("ПЕРЕСТАНОВОЧНЫЙ ТЕСТ (CLR + Aitchison) - БЕЗ БАЛАНСИРОВКИ\n")
    f.write("ВНИМАНИЕ: Результаты искажены из-за разного размера групп!\n")
    f.write("=" * 60 + "\n\n")
    f.write(f"Число перестановок: {n_permutations}\n")
    f.write(f"Размер групп (исходный): здоровых={n_healthy}, CRC={n_crc}\n\n")
...
print("\nРезультаты сохранены в perm_results_clr_no_balance.txt")
print("Готово!")
